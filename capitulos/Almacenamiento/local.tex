...
\section{Procedimientos de procesamiento de datos}
...
\subsection{Procedimiento: Agrupación Pasos Por intervalos}
\label{sub:sec:procedimientosSQL}
...
\centering
\begin{lstlisting}[basicstyle={\scriptsize\ttfamily},breaklines=true,language=SQL,numbers=left,numberstyle={\tiny},stepnumber=1,tabsize=6]
CREATE DEFINER=`root`@`localhost` PROCEDURE `agrupaPasosPorIntervalosNodo`(in fechaMIN Varchar(40), in fechaMAX Varchar(40), in intervalo INT, in nodo BIGINT(20) )
BEGIN
	DECLARE inter INT DEFAULT intervalo*1000*60;
	SELECT FROM_UNIXTIME(round(tinicio/inter)*inter/1000) as Intervalo ,  count(*)
	FROM paso
		WHERE 
			idNodo = nodo
			AND tinicio
...
END
\end{lstlisting}
\caption[Procedimiento agrupamiento pasos por intervalos I]{Procedimiento para el agrupamiento de los pasos por intervalos de tiempo, para un solo nodo.}
\label{sql:consulta3}
\end{figure}

\subsubsection{Todos los nodos}
...
\centering
\begin{lstlisting}[basicstyle={\scriptsize\ttfamily},breaklines=true,language=SQL,numbers=left,numberstyle={\tiny},stepnumber=1,tabsize=6]
CREATE DEFINER=`root`@`localhost` PROCEDURE `agrupaPasosPorIntervalos`(in fechaMIN Varchar(40), in fechaMAX Varchar(40), in intervalo INT )
BEGIN
	DECLARE inter INT DEFAULT intervalo*1000*60;
	SELECT FROM_UNIXTIME(round(tinicio/inter)*inter/1000) as Intervalo ,  count(*)
	FROM paso
...
END
\end{lstlisting}
\caption[Procedimiento agrupamiento pasos por intervalos II]{Procedimiento para el agrupamiento de los pasos por intervalos de tiempo, para todos los nodos sin distinción entre ellos.}
\label{sql:consulta3}
\end{figure}

\subsubsection{Nodos separados}
...
SELECT SUBSTR(FROM_UNIXTIME(truncate(tinicio/inter,0)*inter/1000),1,corte) as Fecha, __paso.idNodo ,  count(*) as Total, latitud,longitud,nombre,poligono
	FROM __paso, nodo
		WHERE
			tinicio
			BETWEEN  UNIX_TIMESTAMP(fechaMIN)*1000
...
END
\end{lstlisting}
\caption[Procedimiento agrupamiento pasos por intervalos III]{Procedimiento para el agrupamiento de los pasos por intervalos de tiempo, para todos los nodos con distinción entre ellos.}
\label{sql:consulta3}
\end{figure}

\subsection{Procedimiento: Cálculo de trazas para pasos entre nodos}
...
END
\end{lstlisting}
\caption[Procedimiento calculo de trazas]{Procedimiento el cálculo de trazas entre pasos.}
\label{sql:consulta3}
\end{figure}
